...
# Если ссылок в сообщении больше этого числа — все видео скачиваются
# без наложения текста, подписываются цифрами (1, 2, 3...) и отправляются
# одним zip-архивом.
ZIP_THRESHOLD = 2
...
async def generate_text() -> dict:
    ...
    d = json.loads(raw)
    return {"top_text": d.get("top_text", "Drazze меняет правила"), "bottom_text": d.get("bottom_text", "Ссылка в профиле")}
...
def overlay_text(input_path: str, top: str, bottom: str) -> str:
    ...
    vf = (
        ...
        f"drawtext=text='{esc(top)}':fontfile='{font}':fontsize=38:fontcolor=white:x=(w-text_w)/2:y=h*0.04:line_spacing=8:shadowcolor=black@0.6:shadowx=2:shadowy=2,"
        f"drawtext=text='{esc(bottom)}':fontfile='{font}':fontsize=34:fontcolor=white:x=(w-text_w)/2:y=h*0.86:line_spacing=6:shadowcolor=black@0.6:shadowx=2:shadowy=2"
    )
    cmd = ["ffmpeg", "-y", "-i", input_path, "-vf", vf, "-c:v", "libx264", "-c:a", "aac",
...
async def download_tiktok(url: str) -> bytes | None:
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.get(f"https://tikwm.com/api/?url={url}&hd=1", headers={"User-Agent": "Mozilla/5.0"})
        data = r.json()
    if data.get("code") != 0:
        return None
...
def reencode(path: str) -> str:
    ...
    r = subprocess.run(["ffmpeg", "-y", "-i", path, "-c:v", "libx264", "-c:a", "aac",
                         "-movflags", "+faststart", "-pix_fmt", "yuv420p", "-preset", "fast", "-crf", "23", out],
...
async def send_with_overlay(update: Update, video_path: str, top: str, bottom: str):
    ...
    with open(final, "rb") as f:
        await update.effective_message.reply_video(video=f, caption=f"📝 {top}\n{bottom}",
...
async def process_batch_to_zip(update: Update, urls: list[str]):
    """Скачивает несколько видео без наложения текста, подписывает их
    цифрами (1.mp4, 2.mp4, ...) и отправляет одним zip-архивом."""
    count = len(urls)
    status_msg = await update.message.reply_text(f"⏳ Скачиваю 0 из {count} видео...")

    downloaded_for_zip: list[tuple[str, str]] = []
    success = 0
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            ...
            # Подписываем файл просто цифрой — без наложения текста на видео
            downloaded_for_zip.append((f"{i}.mp4", video_path))
            success += 1
    ...
    if downloaded_for_zip:
        await status_msg.edit_text(f"📦 Упаковываю {len(downloaded_for_zip)} видео в zip...")
        zip_path = None
        try:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_STORED) as zf:
                for arcname, path in downloaded_for_zip:
                    zf.write(path, arcname=arcname)

            zip_size_mb = os.path.getsize(zip_path) / (1024 * 1024)
            if zip_size_mb > 50:
                await update.message.reply_text(
                    f"❌ Архив получился слишком большим ({zip_size_mb:.0f} МБ, лимит — 50 МБ). "
                    f"Попробуй отправить ссылки меньшими партиями."
                )
            else:
            ...
            if zip_path and os.path.exists(zip_path):
                try:
                    ...
                    pass
            for _, path in downloaded_for_zip:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not urls:
        await update.message.reply_text(
            f"Отправь ссылку на TikTok 🎬\n\n"
            f"Если прислать больше {ZIP_THRESHOLD} ссылок сразу — скачаю все "
            f"и пришлю одним zip-архивом (видео подписаны цифрами 1, 2, 3...)."
        )
        return

    # Если ссылок больше порога — пакетный режим без интерактива и текста
    if len(urls) > ZIP_THRESHOLD:
        await process_batch_to_zip(update, urls)
        return

    # Старое поведение: берём только первую ссылку, предлагаем выбрать текст
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "skip":
        ...
        with open(video_path, "rb") as f:
            await q.message.reply_video(video=f, supports_streaming=True, write_timeout=120, read_timeout=120)
        os.unlink(video_path)
        ...
        try:
            ...
            pending_video.pop(user_id, None)
            await q.edit_message_text(f"⏳ Накладываю:\n📌 {texts['top_text']}\n📌 {texts['bottom_text']}")
            await send_with_overlay(q, video_path, texts["top_text"], texts["bottom_text"])
            await q.edit_message_text("✅ Готово!")
        except Exception as e:
            await q.edit_message_text(f"❌ Ошибка: {str(e)[:200]}")
    elif action == "custom":
        user_mode[user_id] = "custom"
        await q.edit_message_text("✏️ Введи текст:\n\n`Верхний текст | Нижний текст`\n\nПример:\n`Drazze меняет правила | Ссылка в профиле`", parse_mode="Markdown")
